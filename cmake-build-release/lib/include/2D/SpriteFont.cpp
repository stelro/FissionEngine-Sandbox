/*
    ...
    Modified By: Stelmach Rostislav 2017
*/
...
namespace Fiene {
    ...
    void SpriteFont::startUp(const char *font, int size, char cs, char ce)
    {
        // Initialize SDL_ttf
        if (!TTF_WasInit()) {
            TTF_Init();
        ...
        if (f == nullptr) {
            errorLog("Faild to open TTF font %s\n", font);
            IOManager::iostreamErrorOutput("Faild to open TTF font %s\n", font);
            Exit(1);
        }
        ...
        if (!bestPartition) {
            errorLog("Failed to Map TTF font %s to texture. Try lowering resolution.\n", font);
            throw 282;
        }
        // Create the texture
    ...
    void SpriteFont::startUp(const char *font, int size)
    {
        startUp(font, size, FIRST_PRINTABLE_CHAR, LAST_PRINTABLE_CHAR);
    }
    ...
    void SpriteFont::draw(SpriteBatch& batch, const char* s, glm::vec2 position, glm::vec2 scaling,
        ...
        for (int si = 0; s[si] != 0; si++) {
            ...
            if (s[si] == '\n') {
                ...
                batch.append(Vec4(destRect.x, destRect.y, destRect.z, destRect.w), Vec4(_glyphs[gi].uvRect.x, _glyphs[gi].uvRect.y, _glyphs[gi].uvRect.z, _glyphs[gi].uvRect.w), _texID, depth, tint);
